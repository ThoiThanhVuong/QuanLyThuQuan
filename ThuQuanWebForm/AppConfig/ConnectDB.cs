...
namespace ThuQuanWebForm.AppConfig
{
    class ConnectDB
    {
        ...
        public void OpenConnection()
        {
            ...
            catch (MySqlException ex)
            {
                ...
                // Re-throw the exception so the caller knows there was an error
                throw new InvalidOperationException("Failed to open database connection", ex);
            }
            catch (Exception ex)
            {
                Console.WriteLine("Lỗi kết nối chung: " + ex.Message);
                // Re-throw the exception so the caller knows there was an error
                throw new InvalidOperationException("Unexpected error when opening database connection", ex);
            }
        }
